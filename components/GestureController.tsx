...
interface GestureControllerProps {
  ...
  onFirstHandDetected?: () => void;
}

export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition, onTwoHandsDetected, onFirstHandDetected }) => {
    ...
    const predictWebcam = () => {
      ...
      if (videoRef.current.videoWidth > 0) { // Ensure video is ready
        ...
        if (result.landmarks && result.landmarks.length > 0) {
          // Trigger first hand detection
          if (!hasTriggeredFirstHand.current && onFirstHandDetected) {
